...
def main():
        ...
        for i in lines:
            ...
            product = key[0]
            lot = key[1]
            search = "search/?type=antibody_approval&antibody.product_id="+product+"&antibody.lot_id="+lot+"&frame=embedded"
            object = get_ENCODE(search)
            if object != 'ERROR':
                if len(object['@graph']) == 1:
                     ab = object['@graph'][0]
                     print "\t".join([product, lot, ab['antibody']['accession'], ab['uuid'], ab['status']])
                else:
                    print "FREAK OUT", product, lot
            else:
                print product,'\t', lot
