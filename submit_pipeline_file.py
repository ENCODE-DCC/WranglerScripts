...
VAL = '/data/file-validation-pipeline/dnanexus/validate-files/resources'
DATA = '/opt/data/'
BIN = '/usr/bin/'
...
auth = {}
try:
    auth = json.load(open(VAL+DATA+"keys.json"))
except:
    print "Error loading AUTH keys.  Please add JSON file with AUTHID and AUTHPW named 'keys.json' to (resources)/opt/data"
    exit

encoded_access_key = auth['AUTHID']
encoded_secret_access_key = auth['AUTHPW']

my_lab = '/labs/richard-myers/'
my_award = '/awards/U54HG006998/'

# From http://hgwdev.cse.ucsc.edu/~galt/encode3/validatePackage/validateEncode3-latest.tgz
encValData =VAL+DATA+'encValData'
assembly = 'hg19'

if (len(sys.argv) != 4):
   print "Usage: %s /path/to/file format ENCSR###ZZZ"
   exit(0)
...
if validate_args is not None:
    ...
    try:
        subprocess.check_output([VAL+BIN+'validateFiles'] + validate_args + [path])
...
rg = requests.get(host+'/experiments/ENCSR765JPC/', headers=headers, auth=(encoded_access_key, encoded_secret_access_key))
print rg.json()
